package com.google.android.gms.ads.internal.reward.mediation.client;

import android.os.Parcel;
import android.os.Parcelable.Creator;
import com.google.android.gms.common.internal.safeparcel.zza;
import com.google.android.gms.common.internal.safeparcel.zza.C0881zza;
import com.google.android.gms.common.internal.safeparcel.zzb;

public class zzc implements Creator<RewardItemParcel> {
    public zzc() {
    }

    static void zza(RewardItemParcel rewardItemParcel, Parcel parcel, int i) {
        RewardItemParcel rewardItemParcel2 = rewardItemParcel;
        Parcel parcel2 = parcel;
        int i2 = i;
        int zzav = zzb.zzav(parcel2);
        zzb.zzc(parcel2, 1, rewardItemParcel2.versionCode);
        zzb.zza(parcel2, 2, rewardItemParcel2.type, false);
        zzb.zzc(parcel2, 3, rewardItemParcel2.zzKS);
        zzb.zzI(parcel2, zzav);
    }

    /* renamed from: zzO */
    public RewardItemParcel[] newArray(int i) {
        return new RewardItemParcel[i];
    }

    /* renamed from: zzo */
    public RewardItemParcel createFromParcel(Parcel parcel) {
        RewardItemParcel rewardItemParcel;
        C0881zza zza;
        StringBuilder sb;
        Parcel parcel2 = parcel;
        int i = 0;
        int zzau = zza.zzau(parcel2);
        String str = null;
        int i2 = 0;
        while (parcel2.dataPosition() < zzau) {
            int zzat = zza.zzat(parcel2);
            switch (zza.zzca(zzat)) {
                case 1:
                    i2 = zza.zzg(parcel2, zzat);
                    break;
                case 2:
                    str = zza.zzp(parcel2, zzat);
                    break;
                case 3:
                    i = zza.zzg(parcel2, zzat);
                    break;
                default:
                    zza.zzb(parcel2, zzat);
                    break;
            }
        }
        if (parcel2.dataPosition() != zzau) {
            C0881zza zza2 = zza;
            StringBuilder sb2 = sb;
            StringBuilder sb3 = new StringBuilder();
            C0881zza zza3 = new C0881zza(sb2.append("Overread allowed size end=").append(zzau).toString(), parcel2);
            throw zza2;
        }
        RewardItemParcel rewardItemParcel2 = rewardItemParcel;
        RewardItemParcel rewardItemParcel3 = new RewardItemParcel(i2, str, i);
        return rewardItemParcel2;
    }
}
